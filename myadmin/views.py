    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None and user.is_staff:
                ...
                form.add_error('', 'Please enter an administrator account')
    else:
        form = UserLoginForm()
...
class VideoPublishView(SuperUserRequiredMixin, generic.UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        clf_list = list(Classification.objects.all().values())
...
def video_delete(request):
    if not request.user.is_superuser:
        return JsonResponse({"code": 1, "msg": "No delete permission"})
...
def user_delete(request):
    ...
    if instance.is_superuser:
        return JsonResponse({"code": 1, "msg": "Cannot delete administrator"})
    instance.delete()
    return JsonResponse({"code": 0, "msg": "success"})
...
class SubscribeView(SuperUserRequiredMixin, generic.View):
    ...
    def post(self, request):
        if not request.user.is_superuser:
            return JsonResponse({"code": 1, "msg": "No permission"})
        video_id = request.POST['video_id']
        ...
        if email_list:
            for to_list in email_list:
                try:
                    ...
                    logger.error(e)
                    return JsonResponse({"code": 1, "msg": "Failed to send"})
            return JsonResponse({"code": 0, "msg": "success"})
        else:
            return JsonResponse({"code": 1, "msg": "Mailing list is empty"})
