    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not user.is_staff and not user.is_superuser:
                user.save()
                update_session_auth_hash(request, user)  # Update session is very important!
                messages.success(request, 'Successfully modified')
                return redirect('users:change_password')
            else:
                messages.warning(request, 'No right to modify the administrator password')
                return redirect('users:change_password')
        else:
            print((form.errors))
    else:
...
class FeedbackView(LoginRequiredMixin, generic.CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if was_limited:
            messages.warning(self.request, "The operation is too frequent,please try again in 1 minute")
            return render(request, 'users/feedback.html', {'form': FeedbackForm()})
        return super().post(request, *args, **kwargs)

    def get_success_url(self):
        messages.success(self.request, "Submitted successfully")
